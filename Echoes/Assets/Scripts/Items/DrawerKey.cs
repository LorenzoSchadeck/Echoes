...
/// <summary>
/// Chave que pode ser coletada para desbloquear gavetas específicas.
/// Implementa IInteractable para permitir coleta através do sistema de interação padrão.
/// </summary>
public class DrawerKey : MonoBehaviour, IInteractable
{
    [Header("🔑 Key Settings")]
    [Tooltip("ID único desta chave - deve corresponder ao ID da gaveta que ela desbloqueia")]
    ...
    [Tooltip("Se verdadeiro, desabilita visualmente o objeto mas mantém na cena")]
    ...
    public string InteractionPrompt => 
        pickupPrompt?.GetLocalizedString() ?? $"Pegar Chave";
    ...
    private void CollectKey()
    {
        ...
        // Notifica o gerenciador de chaves
        DrawerKeyManager.Instance?.RegisterCollectedKey(keyID);
        
        // Dispara evento de legenda
        GameEvents.TriggerDrawerKeyCollected();
        
        // Feedback visual
        if (disableRenderersOnPickup)
        {
            DisableVisuals();
        }
    ...
    /// <summary>
    /// Desabilita os renderizadores do objeto
    /// </summary>
    private void DisableVisuals()
    {
        ...
        // Também desabilita o collider para não ser detectado novamente
    ...
    private void ValidateConfiguration()
    {
        ...
        {
            Debug.LogWarning($"[DrawerKey] {gameObject.name}: keyID não foi configurado! Esta chave não desbloqueará nenhuma gaveta.", this);
        }
        
        if (interactionDistance <= 0f)
        {
            Debug.LogWarning($"[DrawerKey] {gameObject.name}: interactionDistance deve ser maior que zero!", this);
        }
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        UnityEditor.Handles.Label(transform.position + Vector3.up * 0.3f, 
            $"Chave: {(string.IsNullOrEmpty(keyID) ? "SEM ID!" : keyID)}");
    ...
    private void TestCollectKey()
    {
        ...
        {
            Debug.Log("[DrawerKey] Teste de coleta só funciona em runtime!");
            return;
        }
        
        CollectKey();
        Debug.Log($"[DrawerKey] {gameObject.name} com ID '{keyID}' foi coletada!");
    }
